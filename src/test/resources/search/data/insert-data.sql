...
INSERT INTO recipe (is_public, name, description, user_id, created_at) VALUES
                                                    (true, 'Protein-Packed Breakfast Bowl', 'A healthy bowl with eggs, yogurt, and fruit.', (SELECT id FROM user WHERE username = 'testuser'), NOW()),
                                                    (true, 'Grilled Chicken & Veggies', 'Simple grilled chicken breast with roasted broccoli.', (SELECT id FROM user WHERE username = 'testuser'), NOW()),
                                                    (true, 'Salmon & Rice Dinner', 'Baked salmon served with brown rice and steamed vegetables.', (SELECT id FROM user WHERE username = 'testuser'), NOW());

INSERT INTO plan (is_public, name, description, plan_type_id, user_id, created_at) VALUES
                                                                (true, 'Beginner Full Body Workout', 'A simple plan for starting out.',  (SELECT id FROM plan_type WHERE name = 'Workout Plan'),(SELECT id FROM user WHERE username = 'testuser'), NOW()),
                                                                (true, 'High-Protein Nutrition Guide', 'Meal ideas focusing on protein intake.', (SELECT id FROM plan_type WHERE name = 'Nutrition Plan'), (SELECT id FROM user WHERE username = 'testuser'), NOW()),
                                                                (true, 'Fat Loss Hybrid Plan', 'Combines workouts and diet for fat loss.',(SELECT id FROM plan_type WHERE name = 'Hybrid Plan'), (SELECT id FROM user WHERE username = 'testuser'), NOW());
