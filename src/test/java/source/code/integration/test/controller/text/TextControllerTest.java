...
public class TextControllerTest {
	...
	void getAllTextForExerciseInstructionForbidden() throws Exception {
		...
		mockMvc.perform(get("/api/text/{parentId}/type/{type}", parentId, type)).andExpectAll(status().isForbidden());
	}
	...
	void getAllTextForParentNotFound() throws Exception {
		...
		mockMvc.perform(get("/api/text/{parentId}/type/{type}", parentId, type)).andExpectAll(status().isNotFound());
	}
	...
	void updateExerciseInstructionText() throws Exception {
		...
		mockMvc
			.perform(patch("/api/text/{id}/type/{type}", id, type).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(updateDto)))
			.andExpectAll(status().isNoContent());

		mockMvc.perform(get("/api/text/{id}/type/{type}", id, type))
			.andExpectAll(status().isOk(), jsonPath("$[0].text").value("Updated text"));
	}
	...
	void deleteExerciseInstructionText() throws Exception {
		...
		mockMvc.perform(delete("/api/text/{id}/type/{type}", id, type)).andExpectAll(status().isNoContent());
	}
	...
	void deleteTextForbidden() throws Exception {
		...
		mockMvc.perform(delete("/api/text/{id}/type/{type}", id, type)).andExpectAll(status().isForbidden());
	}
	...
	void deleteTextNotFound() throws Exception {
		...
		mockMvc.perform(delete("/api/text/{id}/type/{type}", id, type)).andExpectAll(status().isNotFound());
	}
