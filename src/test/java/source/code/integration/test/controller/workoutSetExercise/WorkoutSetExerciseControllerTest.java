...
public class WorkoutSetExerciseControllerTest {
	...
	void createWorkoutSetExercise() throws Exception {
		...
		var request = new WorkoutSetExerciseCreateDto(new BigDecimal("25.0"), (short) 12, 1, 1, (short) 1);

		mockMvc
			.perform(post("/api/workout-set-exercises").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request)))
			.andExpectAll(status().isCreated(), jsonPath("$.id").exists(), jsonPath("$.weight").value(25.0),
					jsonPath("$.repetitions").value(12), jsonPath("$.orderIndex").exists(),
					jsonPath("$.exerciseId").value(1), jsonPath("$.exerciseName").exists());
	}
	...
	void updateWorkoutSetExercise() throws Exception {
		...
		mockMvc
			.perform(
					patch("/api/workout-set-exercises/{id}", id).contentType(MediaType.APPLICATION_JSON).content(patch))
	...
	void deleteWorkoutSetExercise() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-set-exercises/{id}", id)).andExpectAll(status().isNoContent());

		mockMvc.perform(get("/api/workout-set-exercises/{id}", id)).andExpect(status().isNotFound());
	}
	...
	void deleteWorkoutSetExerciseNotOwnerOrAdmin() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-set-exercises/{id}", id)).andExpectAll(status().isForbidden());
	}
	...
	void deleteWorkoutSetExerciseNotFound() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-set-exercises/{id}", id)).andExpectAll(status().isNotFound());
	}
	...
	void getWorkoutSetExercisePublic() throws Exception {
		...
		mockMvc.perform(get("/api/workout-set-exercises/{id}", id))
			.andExpectAll(status().isOk(), jsonPath("$.id").value(id), jsonPath("$.weight").value(20.0),
					jsonPath("$.repetitions").value(10), jsonPath("$.orderIndex").exists(),
					jsonPath("$.exerciseId").value(1), jsonPath("$.exerciseName").exists());
	}
	...
	void getAllWorkoutSetExercisesForWorkoutSet() throws Exception {
		...
		mockMvc.perform(get("/api/workout-set-exercises/workout-sets/{workoutSetId}", workoutSetId))
			.andExpectAll(status().isOk(), jsonPath("$").isArray(), jsonPath("$[0].id").value(1),
					jsonPath("$[0].orderIndex").exists(), jsonPath("$[0].weight").value(20.0),
					jsonPath("$[0].repetitions").value(10), jsonPath("$[0].exerciseId").value(1),
