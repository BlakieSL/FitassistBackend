...
@SqlGroup({
        @Sql(scripts = "classpath:workoutSetGroup/data/insert-data.sql.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD),
        @Sql(scripts = "classpath:workoutSetGroup/data/remove-data.sql", executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
})
public @interface WorkoutSetGroupSql {
}
