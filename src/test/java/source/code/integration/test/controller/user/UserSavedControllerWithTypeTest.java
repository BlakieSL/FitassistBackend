...
public class UserSavedControllerWithTypeTest {
	...
	void getAllFromUserSavePlan() throws Exception {
		mockMvc.perform(get("/api/user-saved/item-type/PLAN/type/SAVE/user/1"))
			.andExpectAll(status().isOk(), jsonPath("$.content").value(hasSize(3)),
	...
	void getAllFromUserDislikeComment() throws Exception {
		mockMvc.perform(get("/api/user-saved/item-type/COMMENT/type/DISLIKE/user/1"))
			.andExpectAll(status().isOk(), jsonPath("$.content", hasSize(2)), jsonPath("$.content[0].id").value(2),
					jsonPath("$.content[1].id").value(1), jsonPath("$.page.totalElements").value(2));
	}
	...
	void getAllFromUserDislikePlan() throws Exception {
		mockMvc.perform(get("/api/user-saved/item-type/PLAN/type/DISLIKE/user/2"))
			.andExpectAll(status().isOk(), jsonPath("$.content", hasSize(2)), jsonPath("$.content[0].id").value(3),
					jsonPath("$.content[1].id").value(1), jsonPath("$.page.totalElements").value(2));
	}
	...
	void getAllFromUserSortDesc() throws Exception {
		mockMvc.perform(get("/api/user-saved/item-type/PLAN/type/SAVE/user/1").param("sort", "createdAt,DESC"))
			.andExpectAll(status().isOk(), jsonPath("$.content").value(hasSize(3)),
					jsonPath("$.content[0].id").value(3), jsonPath("$.content[1].id").value(2),
	...
	void getAllFromUserSortAsc() throws Exception {
		mockMvc.perform(get("/api/user-saved/item-type/PLAN/type/SAVE/user/1").param("sort", "createdAt,ASC"))
			.andExpectAll(status().isOk(), jsonPath("$.content").value(hasSize(3)),
					jsonPath("$.content[0].id").value(1), jsonPath("$.content[1].id").value(2),
	...
	void getAllFromUserDefaultSort() throws Exception {
		Utils.setUserContext(1);
		mockMvc.perform(get("/api/user-saved/item-type/COMMENT/type/LIKE/user/1"))
			.andExpectAll(status().isOk(), jsonPath("$.content").exists(), jsonPath("$.page.totalElements").exists());
	}
