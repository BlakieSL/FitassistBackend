...
@TestPropertySource(properties = "schema.name=forumThread")
...
public class ForumThreadControllerTest {
    ...
    public void createForumThread_ShouldCreateForumThread() throws Exception {
        ...
        mockMvc.perform(post("/api/threads")
                ...
                .andExpectAll(
                        status().isOk(),
                        jsonPath("$.id").exists(),
                        jsonPath("$.title").value("New Thread Title"),
                        jsonPath("$.text").value("This is the content of the new thread"),
                        jsonPath("$.threadCategoryId").value(1),
    ...
    void deleteForumThread_ShouldReturn404_WhenForumThreadDoesNotExist() throws Exception {
        ...
        mockMvc.perform(delete("/api/threads/9999"))
    ...
    void getForumThread_ShouldReturnForumThreadById() throws Exception {
        mockMvc.perform(get("/api/threads/1"))
                .andExpectAll(
                        status().isOk(),
                        jsonPath("$.id").value(1),
                        jsonPath("$.title").value("Favorite Apps")
                );
    }
    ...
    void getForumThread_ShouldReturn404_WhenForumThreadDoesNotExist() throws Exception {
        mockMvc.perform(get("/api/threads/9999"))
                .andExpect(status().isNotFound());
    }
    ...
    void getAllForumThreads_ShouldReturnAllForumThreads() throws Exception {
        mockMvc.perform(get("/api/threads"))
                .andExpectAll(
                        ...
                        jsonPath("$[0].title").value("Favorite Apps")
                );
    }
