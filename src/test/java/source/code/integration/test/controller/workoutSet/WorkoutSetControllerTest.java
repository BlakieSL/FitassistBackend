...
public class WorkoutSetControllerTest {
	...
	void createWorkoutSet() throws Exception {
		...
		var request = new WorkoutSetCreateDto((short) 1, (short) 60, 1);

		mockMvc
			.perform(post("/api/workout-sets").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(request)))
			.andExpectAll(status().isCreated(), jsonPath("$.id").exists(), jsonPath("$.orderIndex").value(1),
					jsonPath("$.restSeconds").value(60), jsonPath("$.workoutSetExercises").isArray());
	}
	...
	void updateWorkoutSet() throws Exception {
		...
		mockMvc.perform(patch("/api/workout-sets/{id}", id).contentType(MediaType.APPLICATION_JSON).content(patch))
	...
	void deleteWorkoutSet() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-sets/{id}", id)).andExpectAll(status().isNoContent());

		mockMvc.perform(get("/api/workout-sets/{id}", id)).andExpect(status().isNotFound());
	}
	...
	void deleteWorkoutSetNotFound() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-sets/{id}", id)).andExpectAll(status().isNotFound());
	}
	...
	void deleteWorkoutSetForbidden() throws Exception {
		...
		mockMvc.perform(delete("/api/workout-sets/{id}", id)).andExpectAll(status().isForbidden());
	}
	...
	void getWorkoutSetPublic() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/{id}", id))
			.andExpectAll(status().isOk(), jsonPath("$.id").value(id), jsonPath("$.orderIndex").value(1),
					jsonPath("$.restSeconds").value(60), jsonPath("$.workoutSetExercises").isArray());
	}
	...
	void getWorkoutSetForbidden() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/{id}", id)).andExpectAll(status().isForbidden());
	}
	...
	void getWorkoutSetNotFound() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/{id}", id)).andExpectAll(status().isNotFound());
	}
	...
	void getAllWorkoutSetsForWorkout() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/workouts/{workoutId}", workoutId))
			.andExpectAll(status().isOk(), jsonPath("$").isArray(), jsonPath("$[0].orderIndex").value(1),
					jsonPath("$[0].restSeconds").value(60), jsonPath("$[0].workoutSetExercises").isArray());
	}
	...
	void getAllWorkoutSetsForWorkoutOwner() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/workouts/{workoutId}", workoutId))
			.andExpectAll(status().isOk(), jsonPath("$").isArray(), jsonPath("$[0].orderIndex").value(1),
					jsonPath("$[0].restSeconds").value(90), jsonPath("$[0].workoutSetExercises").isArray());
	}
	...
	void getAllWorkoutSetsForWorkoutForbidden() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/workouts/{workoutId}", workoutId)).andExpectAll(status().isForbidden());
	}
	...
	void getAllWorkoutSetsForWorkoutEmpty() throws Exception {
		...
		mockMvc.perform(get("/api/workout-sets/workouts/{workoutId}", workoutId)).andExpectAll(status().isNotFound());
	}
