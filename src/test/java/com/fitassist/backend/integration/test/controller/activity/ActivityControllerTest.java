package com.fitassist.backend.integration.test.controller.activity;
...
public class ActivityControllerTest {
	...
	void createActivityAdmin() throws Exception {
		...
		mockMvc
			...
			.andExpectAll(status().isCreated(), jsonPath("$.id").exists(), jsonPath("$.name").value("Swimming"),
					jsonPath("$.met").value(7.5), jsonPath("$.category.id").value(1));
	}
	...
	void deleteActivityAdmin() throws Exception {
		...
		mockMvc.perform(delete("/api/activities/1")).andExpectAll(status().isNoContent());
	}
	...
	void deleteActivityNotAdmin() throws Exception {
		...
		mockMvc.perform(delete("/api/activities/1")).andExpectAll(status().isForbidden());
	}
	...
	void deleteActivityNotFound() throws Exception {
		...
		mockMvc.perform(delete("/api/activities/999")).andExpectAll(status().isNotFound());
	}
	...
	void getActivity_ShouldReturnActivityWithSavedTrue_WhenUserHasSavedIt() throws Exception {
		...
		mockMvc.perform(get("/api/activities/1"))
			.andExpectAll(status().isOk(), jsonPath("$.id").value(1), jsonPath("$.name").value("Brisk Walking"),
					jsonPath("$.met").value(3.5), jsonPath("$.category.id").value(1),
					jsonPath("$.category.name").value("Walking"), jsonPath("$.images..imageUrls").isArray(),
					jsonPath("$.savesCount").value(1), jsonPath("$.saved").value(true));
	}
	...
	void getActivityNotFound() throws Exception {
		mockMvc.perform(get("/api/activities/999")).andExpectAll(status().isNotFound());
	}
	...
	void getFilteredActivitiesByCategory() throws Exception {
		FilterCriteria criteria = FilterCriteria.of("CATEGORY", 2, FilterOperation.EQUAL);
		FilterDto filterDto = FilterDto.of(List.of(criteria), FilterDataOption.AND);

		mockMvc
			.perform(post("/api/activities/filter").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(filterDto)))
			.andExpectAll(status().isOk(), jsonPath("$.content[0].id").exists(),
					jsonPath("$.content[0].name").value("Jogging"), jsonPath("$.content[0].met").value(6.0),
	...
	void getFilteredActivitiesByMet() throws Exception {
		FilterCriteria criteria = FilterCriteria.of("MET", BigDecimal.valueOf(7.5), FilterOperation.GREATER_THAN_EQUAL);
		FilterDto filterDto = FilterDto.of(List.of(criteria), FilterDataOption.AND);
	...
	void getFilteredActivitiesInvalidFilterKey() throws Exception {
		FilterCriteria criteria = FilterCriteria.of("invalidKey", 2, FilterOperation.EQUAL);
		FilterDto filterDto = FilterDto.of(List.of(criteria), FilterDataOption.AND);
	...
	void getFilteredActivitiesInvalidFilterValue() throws Exception {
		FilterCriteria criteria = FilterCriteria.of("CATEGORY", "invalidValue", FilterOperation.EQUAL);
		FilterDto filterDto = FilterDto.of(List.of(criteria), FilterDataOption.AND);
	...
	void getFilteredActivitiesInvalidOperation() throws Exception {
		...
		mockMvc.perform(post("/api/activities/filter").contentType(MediaType.APPLICATION_JSON).content(requestJson))
			.andExpectAll(status().isBadRequest());
	}
	...
	void calculateActivityCaloriesBurned() throws Exception {
		...
		mockMvc
			...
			.andExpectAll(status().isOk(), jsonPath("$.caloriesBurned").hasJsonPath());
	}
