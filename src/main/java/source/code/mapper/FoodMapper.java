...
public abstract class FoodMapper {
    ...
    protected void calculateMacros(@MappingTarget FoodCalculatedMacrosResponseDto dto,
                                   Food food, @Context BigDecimal factor) {
        MathContext mathContext = new MathContext(10, RoundingMode.HALF_UP);

        BigDecimal calories = food.getCalories().multiply(factor, mathContext)
                .setScale(1, RoundingMode.HALF_UP);
        BigDecimal protein = food.getProtein().multiply(factor, mathContext)
                .setScale(1, RoundingMode.HALF_UP);
        BigDecimal fat = food.getFat().multiply(factor, mathContext)
                .setScale(1, RoundingMode.HALF_UP);
        BigDecimal carbohydrates = food.getCarbohydrates().multiply(factor, mathContext)
                .setScale(1, RoundingMode.HALF_UP);
