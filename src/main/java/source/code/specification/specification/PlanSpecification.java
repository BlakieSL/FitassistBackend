...
public class PlanSpecification extends AbstractSpecification<Plan, PlanField> {
    ...
    protected Predicate buildPredicateForField(PredicateContext<Plan> context, PlanField field) {
        return switch (field) {
            ...
            case SAVED_BY_USER -> GenericSpecificationHelper.buildSavedByUserPredicate(
                    context, LikesAndSaves.USER_PLANS.getFieldName(), TYPE_FIELD, TypeOfInteraction.SAVE);
            case LIKED_BY_USER -> GenericSpecificationHelper.buildSavedByUserPredicate(
                    context, LikesAndSaves.USER_PLANS.getFieldName(), TYPE_FIELD, TypeOfInteraction.LIKE);
            case DISLIKED_BY_USER -> GenericSpecificationHelper.buildSavedByUserPredicate(
                    context, LikesAndSaves.USER_PLANS.getFieldName(), TYPE_FIELD, TypeOfInteraction.DISLIKE);
    ...
    private Predicate buildCategoryPredicate(PredicateContext<Plan> context) {
        Join<Plan, PlanCategoryAssociation> categoryAssociationJoin =
                context.root().join(PLAN_CATEGORY_ASSOCIATIONS_FIELD, JoinType.LEFT);

        return GenericSpecificationHelper.buildPredicateJoinProperty(
                context, categoryAssociationJoin, PlanCategoryAssociation.PLAN_CATEGORY);
    }
    ...
    private Predicate buildEquipmentPredicate(PredicateContext<Plan> context) {
        ...
        subquery.select(subRoot.get(ID_FIELD))
                .where(context.builder().and(
                        context.builder().equal(subRoot.get(ID_FIELD), context.root().get(ID_FIELD)),
