    ...
    protected Predicate buildPredicateForField(PredicateContext<ForumThread> context, ForumThreadField field) {
        return switch (field) {
            ...
            case SAVE -> GenericSpecificationHelper.buildPredicateUserEntityInteractionRange(
                    context, LikesAndSaves.USER_THREADS.getFieldName(), TYPE_FIELD, TypeOfInteraction.SAVE);
            case COMMENTS -> GenericSpecificationHelper.buildPredicateUserEntityInteractionRange(
                    context, COMMENTS_FIELD, null, null);
        };
    }
}
