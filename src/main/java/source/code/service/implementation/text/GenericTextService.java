...
public abstract class GenericTextService<T, R, U, E extends JpaRepository<T, Integer>> {
	...
	public void deleteText(int id) {
		...
		applicationEventPublisher
			.publishEvent(TextClearCacheEvent.of(this, textCacheKeyGenerator.generateCacheKey(entity)));
	}

	@Transactional
	public void updateText(int id, JsonMergePatch patch) throws JsonPatchException, JsonProcessingException {
		T entity = findById(id);
	...
	private T findById(int id) {
		return repository.findById(id).orElseThrow(() -> RecordNotFoundException.of(getClass(), id));
	}

	private U applyPatch(JsonMergePatch patch) throws JsonPatchException, JsonProcessingException {
	...
	private Optional<List<BaseTextResponseDto>> getCachedText(String cacheKey) {
		...
		return Optional.ofNullable(cachedValue).map(value -> ((List<BaseTextResponseDto>) cachedValue.get()));
	}
