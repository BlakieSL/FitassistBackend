...
public interface UserPlanRepository extends JpaRepository<UserPlan, Integer> {
	...
	@Query("""
			    ...
			    GROUP BY p.id
			""")
	EntityCountsProjection findCountsAndInteractionsByPlanId(@Param("userId") int userId, @Param("planId") int planId);
	...
	@Query(value = """
			    ...
			    AND p.isPublic = true
			""")
	Page<UserPlan> findAllByUserIdAndType(@Param("userId") int userId, @Param("type") TypeOfInteraction type,
			Pageable pageable);
